...
class ContextCompressor:
    ...
    async def _semantic_graph_compression(self, context: str, target_length: int) -> str:
        ...
        selected_indices = sorted(
            sorted(range(len(sentences)), key=lambda i: sentence_scores[i], reverse=True)
...
class ResponseCache:
    ...
    def generate_cache_key(self, request: QueryRequest, model_name: str) -> str:
        ...
        key_data = f"{model_name}:{request.query}:{request.temperature}:{request.max_tokens}"
...
class AnthropicProvider(BaseInferenceProvider):
    ...
    def _calculate_cost(self, usage, model_name: str) -> float:
        ...
        model_pricing = pricing.get(model_name, {"input": 0.003/1000, "output": 0.015/1000})
...
class vLLMProvider(BaseInferenceProvider):
    ...
    async def initialize(self):
        ...
        self.http_client = httpx.AsyncClient(
            ...
            timeout=self.config.get('timeout', 300)  # Longer timeout for local inference
        )
    ...
    async def stream_response(self, request: QueryRequest, model_name: str) -> AsyncIterator[str]:
        ...
        try:
            ...
            async with self.http_client.stream("POST", "/v1/completions", json=payload) as response:
...
class BatchProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_wait_time = config.get('max_wait_time_ms', 50) / 1000  # Convert to seconds
        self.pending_requests = []
        self.batch_lock = asyncio.Lock()
        
    async def add_request(self, request: QueryRequest, provider: BaseInferenceProvider, model_name: str) -> InferenceResponse:
...
class InferenceEngine:
    ...
    async def initialize(self):
        ...
        logger.info(f"Inference engine initialized with {len(self.providers)} providers")
    ...
    async def process_query(self, request: QueryRequest) -> InferenceResponse:
        ...
        request_id = hashlib.md5(f"{request.user_id}:{request.query}:{time.time()}".encode()).hexdigest()
        ...
        try:
            ...
            if request.context and len(request.context) > self.context_compressor.max_context_tokens:
                original_length = len(request.context)
                target_length = int(original_length * self.context_compressor.compression_ratio)
                request.context = await self.context_compressor.compress_context(request.context, target_length)
                compressed_context = True
                logger.info(f"Compressed context from {original_length} to {len(request.context)} chars")
            ...
            INFERENCE_METRICS.errors_total.labels(
                model=routing_decision.selected_model if 'routing_decision' in locals() else "unknown",
    ...
    async def _update_stats(self, model_name: str, response: InferenceResponse, inference_time: float):
