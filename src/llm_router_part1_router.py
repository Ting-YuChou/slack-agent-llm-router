...
from src.utils.schema import QueryRequest, ModelSelection, RoutingDecision
from src.utils.schema import UserTier, QueryType, QueryRequest, RoutingDecision, ModelSelection, ModelConfig
...
class QueryClassifier:
    ...
    def _keyword_classification(self, query: str) -> Dict[QueryType, float]:
        ...
        keywords = {
            QueryType.CODE_GENERATION: ['code', 'function', 'class', 'implement', 'write', 'create', 'develop'],
            QueryType.CODE_ANALYSIS: ['debug', 'fix', 'analyze', 'review', 'optimize', 'refactor'],
            QueryType.ANALYSIS: ['analyze', 'study', 'research', 'examine', 'evaluate', 'assess'],
            QueryType.SUMMARIZATION: ['summarize', 'summary', 'brief', 'overview', 'key points'],
            QueryType.CREATIVE_WRITING: ['story', 'poem', 'creative', 'write', 'compose', 'narrative'],
            QueryType.BRAINSTORMING: ['brainstorm', 'ideas', 'suggestions', 'think', 'creative'],
            QueryType.PLANNING: ['plan', 'strategy', 'roadmap', 'timeline', 'schedule', 'steps'],
            QueryType.MATH: ['calculate', 'solve', 'equation', 'formula', 'math', 'number'],
            QueryType.REASONING: ['why', 'because', 'reasoning', 'logic', 'explain', 'understand']
        }
...
class ModelRouter:
    ...
    async def _build_query_context(self, request: QueryRequest) -> Dict[str, Any]:
        ...
        # Classify query type
        query_type, classification_confidence = self.classifier.classify_query(request.query)
    ...
    def _select_best_model(self, candidates: List[str], context: Dict[str, Any]) -> str:
        ...
        for model_name in candidates:
            ...
            score += max(0, 20 - avg_latency / 100)  # Speed weight (lower latency = higher score)
            ...
            token_count = context['token_count']
            if token_count <= model_config.max_tokens * 0.8:  # 80% utilization threshold
                score += 10
    ...
    def _estimate_cost(self, selection: ModelSelection, context: Dict[str, Any]) -> float:
        ...
        estimated_output_tokens = min(input_tokens * 0.5, context.get('max_tokens', 1000))
