...
class ModelRouter:
    ...
    def _derive_preferred_models_from_context(self, context: Dict[str, Any]) -> List[str]:
        ...
        if bool(context.get("requires_high_reasoning", False)) or context.get(
            "query_complexity"
        ) == "complex":
    ...
    def _select_best_model(self, candidates: List[str], context: Dict[str, Any]) -> str:
        ...
        for model_name in candidates:
            ...
            reliability_weight = 50 if error_sensitivity == "high" else 40
            score += success_rate * reliability_weight  # Reliability weight
            score += max(
            ...
            if cost_per_token > 0:
                ...
                cost_cap = 30 if cost_sensitivity == "high" else 20
                score += min(cost_score, cost_cap)  # Cap cost benefit
            else:
                score += 30 if cost_sensitivity == "high" else 20
