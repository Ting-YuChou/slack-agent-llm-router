...
class vLLMEngine(BaseModelEngine):
    ...
    async def load_model(self):
        ...
        try:
            ...
            if not model_path.exists() and not self.config.model_path.startswith('huggingface.co'):
    ...
    async def generate(self, request: GenerationRequest) -> GenerationResponse:
        ...
        try:
            ...
            output_tokens = len(output.token_ids) if output.token_ids else len(self.tokenizer.encode(generated_text))
...
class HuggingFaceEngine(BaseModelEngine):
    ...
    async def load_model(self):
        ...
        try:
            ...
            self.is_loaded = True
            logger.info(f"Model {self.config.model_name} loaded successfully with HuggingFace")
...
class RemoteModelEngine(BaseModelEngine):
    ...
    def __init__(self, config: ModelConfig, api_url: str, api_key: Optional[str] = None):
        super().__init__(config)
    ...
    async def load_model(self):
        ...
        try:
            ...
            logger.error(f"Failed to connect to remote model {self.config.model_name}: {e}")
            self.is_loaded = False
            raise
...
class ModelManager:
    ...
    def register_model(self, model_name: str, config: ModelConfig, engine_type: str = "vllm", **kwargs):
    ...
    async def stream_generate(self, model_name: str, request: GenerationRequest) -> AsyncIterator[str]:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "models": {name: engine.get_model_info() for name, engine in self.engines.items()},
        }
        ...
        if torch.cuda.is_available():
            info.update({
                "cuda_available": True,
                "gpu_count": torch.cuda.device_count(),
                "total_gpu_memory": [torch.cuda.get_device_properties(i).total_memory for i in range(torch.cuda.device_count())],
                "gpu_memory_allocated": [torch.cuda.memory_allocated(i) for i in range(torch.cuda.device_count())],
                "gpu_memory_reserved": [torch.cuda.memory_reserved(i) for i in range(torch.cuda.device_count())],
            })
        else:
...
def create_mistral_7b_config(model_path: str = "mistralai/Mistral-7B-Instruct-v0.2") -> ModelConfig:
...
def create_llama_7b_config(model_path: str = "meta-llama/Llama-2-7b-chat-hf") -> ModelConfig:
...
def create_codellama_config(model_path: str = "codellama/CodeLlama-7b-Instruct-hf") -> ModelConfig:
