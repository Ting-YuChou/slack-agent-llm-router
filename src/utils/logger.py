...
class ColoredConsoleFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Format message
        formatted = f"{timestamp} {record.levelname:<8} {record.name:<20} {record.getMessage()}"
    ...
    def emit(self, record: logging.LogRecord):
        ...
        try:
            ...
            if level in ['ERROR', 'CRITICAL']:
                ...
                self.metrics['error_counts'][logger_name] = self.metrics['error_counts'].get(logger_name, 0) + 1
...
class SecurityLogger:
    ...
    def log_authentication_attempt(self, user_id: str, success: bool, source_ip: str = None, **context):
    ...
    def log_suspicious_activity(self, user_id: str, activity: str, severity: str = "medium", **context):
        """Log suspicious activity"""
        log_method = getattr(self.logger, severity.lower(), self.logger.warning)
        log_method("Suspicious activity detected", extra={
    ...
    def log_data_access(self, user_id: str, data_type: str, operation: str, **context):
        ...
        self.logger.info("Sensitive data access", extra={
...
class AuditLogger:
    ...
    def log_model_request(self, user_id: str, model_name: str, query_type: str, token_count: int, **context):
        """Log model inference request"""
        self.logger.info("Model request", extra={
    ...
    def log_configuration_change(self, user_id: str, component: str, changes: Dict[str, Any], **context):
    ...
    def log_cost_event(self, user_id: str, model_name: str, cost: float, tokens: int, **context):
        """Log cost-related events"""
        self.logger.info("Cost event", extra={
...
class PerformanceTracker:
    ...
    def track_request_latency(self, endpoint: str, latency_ms: float, **context):
        ...
        self.logger.info("Request latency", extra={
            'metric_type': 'latency',
            'endpoint': endpoint,
    ...
    def track_model_performance(self, model_name: str, latency_ms: float, tokens_per_second: float, **context):
        """Track model performance"""
        self.logger.info("Model performance", extra={
    ...
    def track_resource_usage(self, resource_type: str, usage_percent: float, **context):
        ...
        self.logger.log(level, "Resource usage", extra={
...
class ErrorTracker:
    ...
    def track_api_error(self, endpoint: str, status_code: int, error: Exception, **context):
        """Track API errors"""
        self.track_error(error, {
...
class LoggerFactory:
    ...
    def get_model_logger(model_name: str, **context) -> LoggerAdapter:
        ...
        return get_logger(f"llm_router.models.{model_name}", model=model_name, **context)
...
def log_request_start(logger: LoggerAdapter, method: str, path: str, **context):
    """Log request start"""
    logger.info(f"{method} {path} - Request started", extra={
        'event': 'request_start',
        'http_method': method,
        'path': path,
        **context
...
def log_request_end(logger: LoggerAdapter, method: str, path: str, status_code: int, duration_ms: float, **context):
    """Log request end"""
    level = logging.WARNING if status_code >= 400 else logging.INFO
    logger.log(level, f"{method} {path} - Request completed", extra={
        'event': 'request_end',
        'http_method': method,
        'path': path,
        'status_code': status_code,
...
def log_model_inference(logger: LoggerAdapter, model_name: str, tokens: int, latency_ms: float, cost: float, **context):
    """Log model inference"""
